...
const skillCategories = [
  ...
  },
  {
    ...
    icon: <Terminal size={24} className="text-brand-gold" />,
  ...
  return (
    <section id="skills" ref={sectionRef} className="py-24 pb-32">
      <div className="flex items-center gap-6 mb-16 skills-header">
        <h3 className="text-brand-text whitespace-nowrap">Technical <span className="gold-gradient-text">Skills</span></h3>
        ...
        {skillCategories.map((category, index) => (
          <div key={index} className="skill-card portfolio-card rounded-xl p-8 hover:-translate-y-2 transition-transform duration-500 h-full flex flex-col">
            ...
            <div className="flex items-center gap-4 mb-8">
              <div className="w-12 h-12 rounded-lg bg-brand-gold/10 flex items-center justify-center border border-brand-gold/20">
                {category.icon}
              </div>
            ...
            <div className="flex flex-wrap gap-3 mt-auto">
              {category.skills.map((skill, i) => (
                <span 
                  key={i} 
                  className="px-4 py-2 rounded-lg bg-white/5 border border-white/5 font-stats text-sm text-brand-text/80 hover:bg-brand-gold hover:text-brand-bg hover:border-brand-gold transition-colors duration-300 shadow-sm"
                >
                  {skill}
                </span>
              ))}
